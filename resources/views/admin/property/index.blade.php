        ...
        <div class="col-sm-12">
            <div class="card">
                <div class="card-header">
                    <h4><a href="{{route('propertyowner.create',$user->id)}}" class="btn btn-primary">Add New Property Owner</a></h4>
                </div>
                <div class="table-responsive theme-scrollbar">
                    <table class="table">
                        ...
                        <tbody>
                        @forelse($user->propertis as $property)
                            <tr class="border-bottom-secondary">
                                <th scope="row">{{$property->id}}</th>
                                <td> <img class="img-30 me-2" src="{{ asset('assets/images/dashboard/default/01.png') }}" alt=""><a
                                        href="{{route('propertyowner.show',$property->id)}}">{{$property->code}}</a></td>
                                <td>{{$property->last_name}}</td>
                                <td>{{$property->email}}</td>e
                                <td>{{$property->phone_number}}</td>
                                <td>{{$property->address}}</td>
                                <td>{{$property->gender}}</td>
                                <td>
                                    <form method="post" action="{{route('propertyowner.destroy',[$property->id])}}">
                                        @csrf
                                        @method('DELETE')
                                        <button class="btn btn-danger" type="submit" @if($property->id == Auth::id()) disabled @endif><i class="fa fa-trash"></i></button>
                                    </form>
